...
export function FinancialSummary({ metrics, leads }: Props) {
  ...
  return (
    <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="holo-panel p-6 flex flex-col relative group">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-8 h-8 rounded-lg bg-[#0055FF]/10 flex items-center justify-center">
            <BarChart3 className="w-4 h-4 text-[#0055FF]" />
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-white/5 flex flex-col gap-1">
          ...
          <div className="flex justify-between text-xs">
             ...
             <span className="text-[#00A3FF] font-medium">+ R$ {paidRevenue.toLocaleString('pt-BR')}</span>
          </div>
        </div>
      </div>
    </div>
